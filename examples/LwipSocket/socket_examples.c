...
sockex_nonblocking_connect(void *arg)
{
  ...
  rt_kprintf("closing socket in nonblocking connect succeeded\n");
  ...
  rt_kprintf("select() needed %d ticks to return writable\n", (int)(ticks_b - ticks_a));
  ...
  // rt_kprintf("select() needed %d ticks to return error\n", (int)(ticks_b - ticks_a));
  rt_kprintf("sockex_nonblocking_connect finished successfully\n");
#else
  LWIP_UNUSED_ARG(arg);
#endif
}
...
#define SNDSTR2 "ET / HTTP_1.1\r\n\r\n"
  ...
  rt_kprintf("sockex_testrecv finished successfully\n");
}
...
#define SNDSTR1 "G"
  ...
  sys_thread_new("sockex_select_waiter1", sockex_select_waiter, &h2,
    SOCKET_WAITER_THREAD_STACK_SIZE, SOCKET_WAITER_THREAD_PRIO);
  sys_msleep(100);
  sys_thread_new("sockex_select_waiter2", sockex_select_waiter, &h1,
    SOCKET_WAITER_THREAD_STACK_SIZE, SOCKET_WAITER_THREAD_PRIO);
  sys_msleep(100);
  sys_thread_new("sockex_select_waiter2", sockex_select_waiter, &h4,
    SOCKET_WAITER_THREAD_STACK_SIZE, SOCKET_WAITER_THREAD_PRIO);
  sys_msleep(100);
  sys_thread_new("sockex_select_waiter2", sockex_select_waiter, &h3,
    SOCKET_WAITER_THREAD_STACK_SIZE, SOCKET_WAITER_THREAD_PRIO);
  ...
  rt_kprintf("sockex_testtwoselects finished successfully\n");
}
#else
static void
sockex_testtwoselects(void *arg)
...
socket_example_test(void* arg)
{
  ...
  rt_kprintf("all tests done, thread ending\n");
}
#endif
...
#if SOCKET_EXAMPLES_RUN_PARALLEL
  sys_thread_new("sockex_nonblocking_connect", sockex_nonblocking_connect, &dstaddr,
    SOCKET_EXAMPLE1_THREAD_STACK_SIZE, SOCKET_EXAMPLE_THREAD_PRIO);
  sys_thread_new("sockex_testrecv", sockex_testrecv, &dstaddr,
    SOCKET_EXAMPLE2_THREAD_STACK_SIZE, SOCKET_EXAMPLE_THREAD_PRIO);
  sys_thread_new("sockex_testtwoselects", sockex_testtwoselects, &dstaddr,
    SOCKET_EXAMPLE3_THREAD_STACK_SIZE, SOCKET_EXAMPLE_THREAD_PRIO);
#else
  sys_thread_new("socket_example_test", socket_example_test, &dstaddr,
    SOCKET_EXAMPLE_THREAD_STACK_SIZE, SOCKET_EXAMPLE_THREAD_PRIO);
#endif
}
